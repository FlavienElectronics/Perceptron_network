...
def write_layer_weights_mem(npz_path, layer_name="L1", data_width=8, data_encoding="integer", test_value=1, test_input_path=None):
    ...
    for neuron_idx in range(n_neurons):
        ...
        for input_idx in range(n_inputs):
            ...
            if data_encoding == "integer":
                ...
                hex_val = int_to_vhdl_hex(weight_value_converted.astype(np.int32), data_width)
        ...
        file_lines.append(f"--  Sum of weights (converted): {int_to_vhdl_hex(np.sum(hex_weights), 16)}")
        file_lines.append("    );")
        file_lines.append("")
...
write_layer_weights_mem("mnist_lone_layer_weights.npz", layer_name="test", data_width=8, data_encoding="integer", test_value=0.0625, test_input_path="mnist_sample_input.npz")
